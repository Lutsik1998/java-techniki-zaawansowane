...
public class MemoryClass {
    ...
    public static synchronized void odsetek() {
        ...
        System.out.println(java.lang.Thread.currentThread().getName() + " Rozmiar pamieci = " + memoryMap.size() + " |  Nieudane odwolania m1 =" + (float) m1 + " |  Nieudane odwolania m2=" + (float) m2 + " | Ogolne odwolania g1 =" + (float) g1 + " | Ogolne odwolania g2=" + (float) g2);

        System.out.println(java.lang.Thread.currentThread().getName() + " Rozmiar pamieci = " + memoryMap.size() + " |  Globalny odsetek=" + (float) m1 / g1 *100 + "%" + " | Od ostatniego raportu=" + (float) m2 / g2 * 100 + "%");
    ...
    public static synchronized void usunZPamici() {

        while (getMemorySize() > RandomLista.getRange()) {
            List<Long> keys = new ArrayList<Long>(memoryMap.keySet());
            memoryMap.remove(keys.get(keys.size()-1));
        }
    }
